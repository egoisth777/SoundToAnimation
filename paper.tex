...
We propose a hierarchical diffusion model architecture, inspired by recent success in large-scale assessment tasks \cite{2208.02205}, to capture both the high-level physical properties (bounciness, damping) and the fine-grained temporal dynamics of collision.
...
\begin{enumerate}
    ...
    \item \textbf{Self-complete Benchmark:} We generated a new dataset, \textit{SoundToAnimation-1k}, containing over 1,000 paired audio-trajectory-material property samples across 50 object categories, for 100 different initial dropping conditions based on \textbf{RealImpact Dataset} and NVIDIA Warp to run the physics simulation and composing the collision sound.
...
To train our multimodal generation model, we constructed \textit{Sound2Motion-1K}, a small scale dataset of paired audio-trajectory-material-properties samples. Unlike existing datasets that rely on simplified physics or motion capture, we leverage high-fidelity rigid body simulation and impulse-based audio synthesis to ensure physical consistency and acoustic realism.

\begin{figure*}[t]
\centering
\includegraphics[width=0.9\linewidth]{figures/dataset-generation-flow.png}
\caption{Overview of the Sound2Animation-1k dataset generation pipeline. The pipeline consists of three main stages: (I) Physics Simulation using Newton XPBD to generate collision events and trajectories, (II) Audio Synthesis that processes collision impulses through sample selection, gain scaling, and spectrogram transformation, and (III) Trajectory Encoding using MDM representation with rotation and velocity features to create the final training samples.}
...
Our final dataset contains over 1,000 paired samples across 50 object categories spanning 100 initial condition. We employ batch simulation with 32 objects per batch and 8 parallel workers, achieving $\sim$1000 samples/hour throughput on an RTX 4090. Trajectories are Z-normalized using dataset statistics to stabilize training, with contact flags remaining unnormalized as binary indicators.
