...
%%%%%%%%% TITLE
\title{Sound2Motion: Multimodal Physics Generation via Hierarchical Diffusion}
...
\subsection{Ablation: Hierarchy Matters}
We found that removing the hierarchical audio encoder (using only a single resolution) degraded Contact Error by 40\%, as the model struggled to resolve the exact frame of impact while maintaining coherent long-term motion.
